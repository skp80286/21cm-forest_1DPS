...
class ModelTester:
    ...
    def test(self, los_test, p_test, stats_test, bispectrum_set, y_test, los_so, silent=True):
        ...
        with torch.no_grad():
            ...
            rms_score = np.mean(mean_squared_error(y_test, y_pred_np))
...
def validate_filelist(train_files, so_train_files, test_files, so_test_files):
    ...
    for test_file, so_test_file in zip(test_files, so_test_files):
        #print(f'test_file:{test_file}, so_test_file:{so_test_file}')
        test_file_parts = [test_file.rstrip('.dat').split('/')[-1].split('_')[i] for i in indices1]
...
def run(X_train, X_test, y_train, y_train_so, y_test, y_test_so, X_noise, num_epochs, batch_size, lr, kernel1, kernel2, dropout, step, input_points_to_use, showplots=False, saveplots=True, criterion=nn.MSELoss()):
    ...
    model = CnnModel(input_size=len(X_train[0]), input_channels=num_channels, output_size=2, dropout=dropout, step=step, kernel1=kernel1, kernel2=kernel2, latentdim=args.latentdim, pooling=args.pooling, activation=args.activation)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (input_batch, output_batch) in enumerate(dataloader):
            ...
            logger.info(f'predictions:{predictions}\input_batch:{input_batch}\noutput_batch:{output_batch}\nloss:{loss}')
    ...
    model.save_model(f"{output_dir}/cnn_model.pth")  # Save the model to a specified path
...
def objective(trial):
    ...
    params = {
        ...
        'input_points_to_use': 3584,#trial.suggest_int('input_points_to_use', 900, 1400),
    }    
...
if args.runmode in ("train_test", "test_only", "optimize"):
    ...
    if args.runmode == "train_test":
        ...
        model = CnnModel(input_size=args.input_points_to_use, input_channels=1, output_size=2, dropout=args.dropout, step=step, kernel1=args.kernel1, kernel2=args.kernel2, latentdim=args.latentdim, pooling=args.pooling, activation=args.activation)
